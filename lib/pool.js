...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    **/
    this.updateFee = function(newFee) {
        emitLog('Updating pool fee: ' + newFee);
        SetupRecipients(newFee);
    };
    ...
    function SetupRecipients(customFee){
        var recipients = [];
        var fee_updated = false;
        ...
        for (var r in options.rewardRecipients){
            ...
            if ( customFee >= 0 && fee_updated == false ){
                percent = parseInt(customFee);
                fee_updated = true;
            }                                 
            ...
            if (!isValidBlock)
                ...
                if (jobManagerLastSubmitBlockHex === blockHex) {
                    ...
                    SubmitBlock(blockHex, function(){
                        CheckBlockAccepted(shareData.blockHash, function(isAccepted, tx, height){
